...
namespace Unity.Muse.Animate
{
    class SceneViewPlayArea : Image
    {
        ...
        {
            m_AuthorContext.CameraContext.CameraMovement.DollyScroll(-obj.delta.y);
        }
        ...
        public void LateUpdate()
        {
            ...
            UpdateTextureDimensions();
            m_AuthorContext.CameraContext.Camera.RenderToTexture(m_RenderTexture, true);
            image = m_RenderTexture;
        }

        void UpdateTextureDimensions()
        {
            var dpi = Vector2.one * Mathf.Max(Unity.AppUI.Core.Platform.mainScreenScale, 1f);

            if (m_AuthorContext == null)
            {
                if (!m_RenderTexture)
                {
                    m_RenderTexture = new RenderTexture(8, 8, 0) { hideFlags = HideFlags.HideAndDontSave };
                }

                return;
            }

            dpi.x = Mathf.Max(dpi.x, m_AuthorContext.Camera.RenderScaling.x, Unity.AppUI.Core.Platform.mainScreenScale);
            dpi.y = Mathf.Max(dpi.y, m_AuthorContext.Camera.RenderScaling.y, Unity.AppUI.Core.Platform.mainScreenScale);
            ...
            if (w <= 0 || h <= 0)
            {
                m_RenderTexture = new RenderTexture(8, 8, 24)
                {
            ...
            var needToUpdate = !m_RenderTexture 
                || m_RenderTexture.width != w
                || m_RenderTexture.height != h;

            if (needToUpdate)
            {
                ClearTexture();

                m_RenderTexture = m_AuthorContext != null
                    ? new RenderTexture(w, h, 24)
                    {
                        hideFlags = HideFlags.HideAndDontSave
                    }
                    : new RenderTexture(8, 8, 0)
                    {
                        hideFlags = HideFlags.HideAndDontSave
                    };
            }
        ...
        void RegisterToCamera()
        {
            ...
            m_AuthorContext.CameraContext.Camera.OnViewportRenderScalingChanged += OnCameraViewportRenderScalingChanged;
            m_AuthorContext.CameraContext.Camera.OnViewportSizeChanged += OnCameraViewportSizeChanged;
            m_AuthorContext.CameraContext.Camera.OnContextMenu += OnCameraContextMenuOpen;
        ...
        void ClearTexture()
        {
            if (m_RenderTexture)
            {
                m_AuthorContext?.CameraContext.Camera.ClearPermanentRenderTextureTarget();
        ...
        GameObject GetViewportTarget(PointerEventData pointerEventData, out RaycastResult raycastResult)
        {
            ...
            var raycaster = m_AuthorContext.CameraContext.Camera.Target.GetComponent<CustomPhysicsRaycaster>();
