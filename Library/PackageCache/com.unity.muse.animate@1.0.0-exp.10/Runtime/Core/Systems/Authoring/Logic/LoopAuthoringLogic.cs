...
namespace Unity.Muse.Animate
{
    class LoopAuthoringLogic
    {
        ...
        {
            UnregisterKey();
            m_TargetKey = model.HasSelection ? model.GetSelection(0) : null;
            RegisterKey();
        }
        ...
        void RegisterKey()
        {
            ...
            UndoRedoLogic.Instance.TrackModel(m_TargetKey.Key.Loop, model =>
            {
                OnLoopChanged?.Invoke(model);
                UpdateSourceFrame();
            });
        ...
        bool TryGetEntitySourcePose(EntityID entityID, out BakedArmaturePoseModel sourcePose)
        {
            ...
            if (!sourceFrame.TryGetPose(entityID, out sourcePose))
                return false;
