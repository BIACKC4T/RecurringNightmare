...
namespace Unity.Muse.Animate
{
    class PhysicsRequest : IProcessingRequest
    {
        ...
        public void Step()
        {
            ...
            foreach (var pair in m_EntityContexts)
            {
                if (!prevFrame.TryGetPose(pair.Key, out var initialPose))
                {
                ...
                switch (pair.Value.Type)
                {
                    case PhysicsEntityType.Kinematic:
                    {
                        if (!frameToBake.TryGetPose(pair.Key, out var targetPose))
            ...
            foreach (var pair in m_EntityContexts)
            {
                if (!frameToBake.TryGetPose(pair.Key, out var pose))
                {
