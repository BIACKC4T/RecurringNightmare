...
{
    class SequenceView : UITemplateContainer, IUITemplate
    {
        ...
        void RegisterModel()
        {
            ...
            m_Model.OnCurrentTransitionChanged += OnCurrentTransitionChanged;
        ...
        void OnSequenceRequestedKeyContextualMenu(SequenceItemViewModel<TimelineModel.SequenceKey> key)
        {
            ...
            m_SequenceKeyContextualMenu.Open(m_Model.Timeline, m_Model.ClipboardService, m_Model.KeySelection, key, matchingView);
        }
        ...
        void OnCurrentTransitionChanged()
        {
            var currentTransitionIndex = m_Model.CurrentTransitionIndex;

            if (currentTransitionIndex < 0)
                return;

            if (currentTransitionIndex >= m_SequenceTransitionViews.Count ||
                currentTransitionIndex >= m_ScrollView.childCount)
            {
                return;
            }

            // Transition can be detached from scroll-view if a key was just deleted
            if (m_SequenceTransitionViews[currentTransitionIndex].parent == null)
                return;

            m_ScrollView.ScrollTo(m_SequenceTransitionViews[currentTransitionIndex]);
        }
        ...
        void OnItemSelectionChanged()
        {
            ...
            if (m_Model.SelectedTransition != null && m_Model.SelectedTransitionIndex < m_SequenceTransitionViews.Count)
            {
                m_ScrollView.ScrollTo(m_SequenceTransitionViews[m_Model.SelectedTransitionIndex]);
            }
        }
        ...
        {
            foreach (var sequenceKeyView in m_SequenceKeyViews)
            {
                sequenceKeyView.pickingMode = enabled ? PickingMode.Position : PickingMode.Ignore;
            }
