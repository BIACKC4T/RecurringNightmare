...
namespace Unity.Muse.Animate
{
    class MotionToKeysRequest
    {
        ...
        public MotionToKeysService Service { get; }
        public KeySequenceTake Target { get; }

        public MotionToKeysRequest(MotionToKeysService service, KeySequenceTake target)
        {
            Service = service;
            Target = target;
        }
        ...
        public void Start()
        {
            ...
            Service.SamplingLogic.QueueBaking(Target.Sensitivity, Target.UseMotionCompletion);
        }
        ...
        void OnBakingProgressed(BakingLogic logic, float progress)
        {
            
            DevLogger.LogSeverity(TraceLevel.Verbose, "MotionToKeysRequest -> OnBakingProgressed(" + progress + ")");

            State = Status.BakingOutput;
            OnProgressed?.Invoke(this, 0.5f + progress/2f);
        }
        ...
        void OnBakingFailed(BakingLogic logic, string error)
        {
            ...
            DevLogger.LogSeverity(TraceLevel.Verbose, "MotionToKeysRequest -> OnBakingFailed(" + error + ")");
