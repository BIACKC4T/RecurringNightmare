...
namespace Unity.DeepPose.Core
{
    struct CompletionSolverData : IDisposable
    {
        ...
        TensorInt m_InputFrameIndices;
        TensorFloat m_InputPositions;
        TensorFloat m_InputRotations;
        TensorInt m_TargetFrameIndices;
        TensorInt m_InputReferenceIndex;
        ...
        public CompletionSolverData(int numInputFrames, int numTargetFrames, int numJoints, int numOutputContacts, bool transposeOrtho6D, int[] endJoints)
        {
            ...
            // Inputs
            m_InputFrameIndices = TensorUtils.Alloc<TensorInt>( m_NumInputFrames );
            m_TargetFrameIndices = TensorUtils.Alloc<TensorInt>(m_NumTargetFrames );
            m_InputReferenceIndex =TensorUtils.Alloc<TensorInt>( 1 );
            m_InputPositions = TensorUtils.Alloc<TensorFloat>(1, m_NumInputFrames, m_NumInputPositions, 3 );
            m_InputRotations = TensorUtils.Alloc<TensorFloat>(1, m_NumInputFrames, m_NumInputRotations, 4 );
        ...
        {
            var predictedJointPositions = backend.PeekOutput("joint_positions_global") as TensorFloat;
            var predictedJointRotations = backend.PeekOutput("joint_rotations_ortho6d") as TensorFloat;
            var predictedContacts = m_NumOutputContacts > 0 ? backend.PeekOutput("contacts") as TensorFloat : null;
