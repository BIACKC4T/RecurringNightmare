...
namespace Unity.DeepPose.Core
{
    ...
    {
        TensorFloat m_PositionData;
        TensorInt m_PositionIds;
        TensorFloat m_PositionWeights;
        TensorFloat m_PositionTolerances;

        TensorFloat m_RotationData;
        TensorInt m_RotationIds;
        TensorFloat m_RotationWeights;
        TensorFloat m_RotationTolerances;

        TensorFloat m_LookAtData;
        TensorInt m_LookAtIds;
        TensorFloat m_LookAtWeights;
        TensorFloat m_LookAtTolerances;

        TensorFloat m_PosePositions;
        TensorFloat m_PoseRotations;
        TensorInt m_PoseIds;
        ...
        public DeepPoseData(int numJoints, int numPositions, int numRotations, int numLookAts, bool includePose, bool transposeOrtho6d, bool useRaycast, float maxRayDistance)
        {
            ...
            // Tensors
            m_PositionData = TensorUtils.Alloc<TensorFloat>(1, numPositions, posFeatureSize);
            m_PositionIds = TensorUtils.Alloc<TensorInt>(1, numPositions);
            m_PositionWeights = TensorUtils.Alloc<TensorFloat>(1, numPositions);
            m_PositionTolerances = TensorUtils.Alloc<TensorFloat>(1, numPositions);

            m_RotationData = TensorUtils.Alloc<TensorFloat>(1, numRotations, 6);
            m_RotationIds = TensorUtils.Alloc<TensorInt>(1, numRotations);
            m_RotationWeights = TensorUtils.Alloc<TensorFloat>(1, numRotations);
            m_RotationTolerances = TensorUtils.Alloc<TensorFloat>(1, numRotations);

            m_LookAtData = TensorUtils.Alloc<TensorFloat>(1, numLookAts, 6);
            m_LookAtIds = TensorUtils.Alloc<TensorInt>(1, numLookAts);
            m_LookAtWeights = TensorUtils.Alloc<TensorFloat>(1, numLookAts);
            m_LookAtTolerances = TensorUtils.Alloc<TensorFloat>(1, numLookAts);
            
            m_PosePositions = TensorUtils.Alloc<TensorFloat>(1, numJoints, 3);
            m_PoseRotations = TensorUtils.Alloc<TensorFloat>(1, numJoints, 6);
            m_PoseIds = TensorUtils.Alloc<TensorInt>(1, numJoints);
        ...
        public void Evaluate(IModelBackend backend)
        {
            ...
            var predictedHipPosition = backend.PeekOutput("root_joint_position") as TensorFloat;
            var predictedBoneRotations = backend.PeekOutput("joint_rotations") as TensorFloat;
